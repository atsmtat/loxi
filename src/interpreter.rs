use crate::ast;
use crate::ast::{Expr, ExprKind, FunDef, Ident, Lit, Stmt, StmtKind, Visitor};
...
impl Interpreter {
    ...
    fn eval_binary_op(
        ...
        match op_token.token_type {
            TokenType::Plus => match left_val.lox_type {
                LoxType::Double(lnum) => {
            ...
            TokenType::BangEqual => {
                let lox_type = LoxType::Boolean(left_val.truth_value() != right_val.truth_value());
                Ok(LoxValue { lox_type })
            }
        ...
        match expr.expr_kind {
            ...
            ExprKind::LogicalExpr(ref lexpr, ref tok, ref rexpr) => {
                ...
                match &tok.token_type {
                    ...
                    _ => {
                        panic!("unexpected logical operand {:?}", tok);
                    }
                }
                self.visit_expr(rexpr)
            }

            ExprKind::Variable(ref ident) => self.get(ident),
    ...
    fn visit_stmt(&mut self, stmt: &Stmt) -> Self::StmtRet {
        match stmt.stmt_kind {
            ...
            StmtKind::FunStmt(ref fdef) => {
                let lox_val = LoxValue {
                    lox_type: LoxType::Fun(fdef.clone()),
                };
